...
function buildOutputPath(urlString, format, explicitName) {
  ...
  const base = explicitName && explicitName.trim().length > 0 ? explicitName.trim() : `${safeHost}-container-${date}.${format}`;
  return resolve(__dirname, base);
}

async function exportElementToPdf(page, selector, outputPath) {
  const rect = await page.evaluate(sel => {
    ...
    return { left, top, width: Math.ceil(r.width), height: Math.ceil(r.height) };
  }, selector);
...
async function main() {
  ...
  const selector = getArgFromList(argv, '--selector') || '.export-container';
  ...
  if (wantPng) {
    ...
    if (!handle) throw new Error(`未找到元素: ${selector}`);
    await handle.screenshot({ path: outputPath, type: 'png' });
    console.log(`已保存容器 PNG: ${outputPath}`);
  } else if (wantPdf) {
    await exportElementToPdf(page, selector, outputPath);
    console.log(`已保存容器 PDF: ${outputPath}`);
  }
