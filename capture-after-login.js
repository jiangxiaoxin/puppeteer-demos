...
function buildOutputPath(urlString, format, explicitName) {
  ...
  const base = explicitName && explicitName.trim().length > 0 ? explicitName.trim() : `${safeHost}-after-login-${date}.${format}`;
...
async function main() {
  ...
  if (wantPng) {
    ...
    console.log(`已保存登录后页面 PNG: ${outputPath}`);
  } else if (wantPdf) {
    ...
    console.log(`已保存登录后页面 PDF: ${outputPath}`);
  }
